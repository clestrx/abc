...
const Admin_profile_description = () => {
    ...
    const addSliderData = useSelector((state) => state?.category?.addSlider);

    useEffect(() => {
        if (addSliderData?.loading === false) {
            if (addSliderData?.data?.status) {
                router.push('/');
                addSliderData.data = null;
            }
            setLoading(false);
        }
    }, [addSliderData]);
    ...
    const addSlide = () => {
        setSliderData({
            ...sliderData,
            slides: [...sliderData.slides, { description: '' }]
        });
    };
                                ...
                                <div className='bg_img flex justify-center items-center'>
                                    <div className='bg-white p-[40px] w-[80%]'>
                                        ...
                                        <div className='flex items-center'>
                                            ...
                                            <Button className='ml-auto underline' onClick={createSlider}>Create</Button>
                                        </div>
                                    </div>
                                </div>
                            </div>
